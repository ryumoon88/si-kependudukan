        ...
        <div class="content d-flex w-100 justify-content-center align-items-center">
            <form action="{{ route('user.authenticate') }}" class="text-center" method="post">
                ...
                @if (session()->has('error'))
                    <div class="invalid-feedback collapsed">
                        {{ session('error') }}
                    </div>
                @endif
        ...
        var canvasShape = function(block_id, params) {
            ...
            Particle.prototype.Draw = function(ctx) {
                ...
                if (ballShape == 'circle') {
                    ...
                    var tri = [ctx.beginPath(), ctx.moveTo(this.x, this.y), ctx.lineTo(this.x + radius_ball, this
                            .y + radius_ball), ctx.lineTo(this.x + radius_ball, this.y - radius_ball), ctx
                        .closePath(), ctx.fill()
                    ]
                    ...
                    for (side; side < 7; side++) {
                        ctx.lineTo(x + size * Math.cos(side * 2 * Math.PI / 6), y + size * Math.sin(side * 2 * Math
                            .PI / 6));
                    }
                    ctx.fill();
